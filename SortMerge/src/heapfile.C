...
HeapFile::HeapFile( const char *name, Status& returnStatus ) {
	...
	if(status != OK)  {					
		//printf("File does not exist, have to create a new heapfile %s\n", fileName);
		...
		// printf("\n** Creating new heapfile %s, first directory page is %d **\n", fileName, firstDirPageId);
		...
		// printf("Heapfile %s already existed, first directory page is %d\n", fileName, firstDirPageId);
	}
...
//    cout << ">>> enter the destructor." << endl;
    if(!strcmp(fileName, "")) { 
				printf("Deleting heapfile with firstDirPageId %d\n", firstDirPageId);
...
{
		// printf("Attempting to open scan on heapfile %s with firstDirPageId %d\n", fileName, firstDirPageId);
    Scan *s = new Scan(this,status);
		if(status == DONE)
			status = OK;  				// lol
		if(status != OK) 
			printf("openScan returned status %d\n", status);
